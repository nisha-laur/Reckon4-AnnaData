...
export default function ChatBotBox() {
    ...
    const sendMessage = async () => {
        if (message !== '') {
            ...
            await fetch("http://127.0.0.1:5000/chat", requestOption)
                .then(res => res.json())
                .then(data => {
                    setrecData(data['response']);
                    console.log(recData);
                })
            if (recData !== null) {
                AddMessage('reciver', recData);
            }
            else {
                AddMessage('reciver', 'Try again')
            }
        }
        ...
        <div className="body">
            ...
            <div className="chat-bot-box">
                ...
                <div className="chat-enter-box">
                    <input type="text" id='message-box' value={message} onChange={e => setmessage(e.target.value)} />
